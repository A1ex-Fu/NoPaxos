...
# Reproduce figure8 chart from specpaxos paper or fig 5 from nopaxos
...
CSV_EXPORT = True
TOTAL_REQUESTS = 1000
MIN_REQUESTS_PER_CLIENT = 35
CLIENT_COUNTS = [2, 5, 10, 20, 50, 100, 150, 200, 250, 300, 350, 400]
...
def plot_results(results, protocol):
    ...
    for i in range(len(results)):
        label = f"{results[i][0]}" # Use actual client count from results
        plt.annotate(label,
    ...
    # match the axis limits to the paper's figure
    plt.xlim(0, 400000)
    plt.ylim(0, 1200)
...
def main():
    ...
    # Convert dictionary items back to list of tuples for plotting
